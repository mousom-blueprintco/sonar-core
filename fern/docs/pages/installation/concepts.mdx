SonarLabs is a service that wraps a set of AI RAG primitives in a comprehensive set of APIs providing a private, secure, customizable and easy to use GenAI development framework.
...
SonarLabs uses poetry to manage its dependencies. You can install the dependencies for the different setups by running `poetry install --extras "<extra1> <extra2>..."`.
...
SonarLabs uses yaml to define its configuration in files named `settings-<profile>.yaml`.
Different configuration files can be created in the root directory of the project.
SonarLabs will load the configuration at startup from the profile specified in the `SONAR_PROFILES` environment variable.
For example, running:
```bash
...
- `settings-ollama.yaml` is loaded if the `ollama` profile is specified in the `SONAR_PROFILES` environment variable. It can override configuration from the default `settings.yaml`

## About Fully Local Setups
In order to run SonarLabs in a fully local setup, you will need to run the LLM, Embeddings and Vector Store locally.
### Vector stores
...
* (Recommended) You can use the 'ollama' option in SonarLabs, which will connect to your local Ollama instance. Ollama simplifies a lot the installation of local LLMs.
* You can use the 'embeddings-huggingface' option in SonarLabs, which will use HuggingFace.
...
* (Recommended) You can use the 'ollama' option in SonarLabs, which will connect to your local Ollama instance. Ollama simplifies a lot the installation of local LLMs.
* You can use the 'llms-llama-cpp' option in SonarLabs, which will use LlamaCPP. It works great on Mac with Metal most of the times (leverages Metal GPU), but it can be tricky in certain Linux and Windows distributions, depending on the GPU. In the installation document you'll find guides and troubleshooting.
