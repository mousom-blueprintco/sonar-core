...
## Base requirements to run SonarLabs

* Clone SonarLabs repository, and navigate to it:

```bash
  git clone https://github.com/itblueprintco/sonarlabs_backend
  cd sonar-labs
```
...
SonarLabs allows to customize the setup -from fully local to cloud based- by deciding the modules to use.
...
In order to only install the required dependencies, SonarLabs offers different `extras` that can be combined during the installation process:
...
> **Important for Windows**: In the examples below or how to run SonarLabs with `make run`, `SONAR_PROFILES` env var is being set inline following Unix command line syntax (works on MacOS and Linux).
If you are using Windows, you'll need to set the env var in a different way, for example:

```powershell
# Powershell
$env:SONAR_PROFILES="ollama"
make run
```
...
```cmd
# CMD
set SONAR_PROFILES=ollama
make run
```

### Local, Ollama-powered setup - RECOMMENDED

**The easiest way to run SonarLabs fully locally** is to depend on Ollama for the LLM. Ollama provides local LLM and Embeddings super easy to install and use, abstracting the complexity of GPU support. It's the recommended setup for local development.
...
Once done, on a different terminal, you can install SonarLabs with the following command:
```bash
poetry install --extras "ui llms-ollama embeddings-ollama vector-stores-qdrant"
```

Once installed, you can run SonarLabs. Make sure you have a working Ollama running locally before running the following command.

```bash
SONAR_PROFILES=ollama make run
```

SonarLabs will use the already existing `settings-ollama.yaml` settings file, which is already configured to use Ollama LLM and Embeddings, and Qdrant. Review it and adapt it to your needs (different models, different Ollama port, etc.)

The UI will be available at http://localhost:8001

### Private, Sagemaker-powered setup

If you need more performance, you can run a version of SonarLabs that relies on powerful AWS Sagemaker machines to serve the LLM and Embeddings.
...
Then, install SonarLabs with the following command:
```bash
...
SonarLabs will use the already existing `settings-sagemaker.yaml` settings file, which is already configured to use Sagemaker LLM and Embeddings endpoints, and Qdrant.

The UI will be available at http://localhost:8001

### Non-Private, OpenAI-powered test setup

If you want to test SonarLabs with OpenAI's LLM and Embeddings -taking into account your data is going to OpenAI!- you can run the following command:
...
Once installed, you can run SonarLabs.

```bash
SONAR_PROFILES=openai make run
```

SonarLabs will use the already existing `settings-openai.yaml` settings file, which is already configured to use OpenAI LLM and Embeddings endpoints, and Qdrant.

The UI will be available at http://localhost:8001

### Non-Private, Azure OpenAI-powered test setup

If you want to test SonarLabs with Azure OpenAI's LLM and Embeddings -taking into account your data is going to Azure OpenAI!- you can run the following command:
...
SonarLabs will use the already existing `settings-azopenai.yaml` settings file, which is already configured to use Azure OpenAI LLM and Embeddings endpoints, and Qdrant.

The UI will be available at http://localhost:8001

### Local, Llama-CPP powered setup

If you want to run SonarLabs fully locally without relying on Ollama, you can run the following command:
...
Once installed, you can run SonarLabs with the following command:

```bash
SONAR_PROFILES=local make run
```

SonarLabs will load the already existing `settings-local.yaml` file, which is already configured to use LlamaCPP LLM, HuggingFace embeddings and Qdrant.

The UI will be available at http://localhost:8001

#### Llama-CPP support

For SonarLabs to run fully locally without Ollama, Llama.cpp is required and in
...
* GPU Virtualization on Windows and OSX: Simply not possible with docker desktop, you have to run the server directly on
  the host.
* Building errors: Some of SonarLabs dependencies need to build native code, and they might fail on some platforms.
