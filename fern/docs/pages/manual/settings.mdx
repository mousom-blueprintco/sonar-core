# Settings and profiles for your Sonar Labs

The configuration of your Sonar Labs server is done thanks to `settings` files (more precisely `settings.yaml`).
...
SonarLabs is configured through *profiles* that are defined using yaml files, and selected through env variables.
...
You have to set environment variable `SONAR_PROFILES` to the name of the profile you want to use.

For example, on **linux and macOS**, this gives:
```bash
export SONAR_PROFILES=my_profile_name_here
```

Windows Powershell(s) have a different syntax, one of them being:
```shell
set SONAR_PROFILES=my_profile_name_here
```
...
### Environment variable `SONAR_PROFILES`
...
For example:
`SONAR_PROFILES=local,cuda` will load `settings-local.yaml`
