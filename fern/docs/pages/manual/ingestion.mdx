...
When you are running SonarLabs in a fully local setup, you can ingest a complete folder for convenience (containing
pdf, text files, etc.)
...
SonarLabs API calls, you may need to include the parameter name before passing the folder path for consumption:
...
You can also use the existing `SONAR_PROFILES=mock` that will set the following configuration for you:
...
time SONAR_PROFILES=mock python ./scripts/ingest_folder.py ~/my-dir/to-ingest/
```
